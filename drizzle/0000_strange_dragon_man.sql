...
CREATE INDEX "movies_fts_idx" ON "movies" USING gin (
    (setweight(to_tsvector('english', "title"), 'A') || setweight(to_tsvector('english', coalesce("overview", '')), 'B')));--> statement-breakpoint
...
CREATE INDEX "series_fts_idx" ON "series" USING gin (
	(setweight(to_tsvector('english', "title"), 'A') || setweight(to_tsvector('english', coalesce("overview", '')), 'B')));--> statement-breakpoint
